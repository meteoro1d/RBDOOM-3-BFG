...
intptr_t idLight::Invoke(const char *functionName, void *param1) {
	...
	if(functionNameHash == 162062) { // GetEditOrigin
		GetEditOrigin();
		return 0;
	};
	...
	if(functionNameHash == 175975) { // GetRenderLight
		GetRenderLight();
		return 0;
	};
	if(functionNameHash == 278761) { // ClientPredictionThink
...
intptr_t iceMonster_Turret::Invoke(const char *functionName, void *param1) {
	int functionNameHash = idStr::Hash(functionName);
	if(functionNameHash == 48744) { // Init
		Init();
		return 0;
...
bool iceMonster_Turret::HasNativeFunction(const char *functionName) {
	int functionNameHash = idStr::Hash(functionName);
	if(functionNameHash == 48744) { // Init
		return true;
	};
